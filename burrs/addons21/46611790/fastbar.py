...
def check_other_addons():
    ...
    ExtendedTagAddon = maybe_import_other_addon(
        "1135507717"
    ...
    def _onBury_45(self):  # self is browser
        ...
        if not all_cards_buried(self, c):
            bury_cards(
                ...
                lambda res: tooltip(f"buried {res.count} cards")
            ).run_in_background()
        else:
            unbury_cards(
                parent=self.mw,
                card_ids=c,
            ).success(lambda res: tooltip("unburied cards")).run_in_background()
...
def make_and_add_toolbar(self):  # self is browser
    ...
    for idx, act_list in enumerate(all_actions):
        if act_list[0] == "action_reschedule_set_due_date":
            all_actions[idx] = [
                "actionReschedule" if anki_21_version < 41 else "action_set_due_date",
                act_list[1],
            ]
    ...
    for idx, val in enumerate(all_actions):
        ...
        icon = qta.icon(
            icon_name,
            color="white" if night_mode_on or theme_manager.night_mode else "black",
        )
        action.setIcon(icon)
    ...
    self.addToolBar(
        fbar
    )  # addToolBar is a method of QMainWindow (that the Browser inherits from)
    self.fbar = fbar
...
def replace_module_name_in_config_help():
    ...
    with open(path, encoding="utf-8") as f:
        ...
        contents = contents.replace(
            "/_addons/46611790", f"/_addons/{mw.addonManager.addonFromModule(__name__)}"
        )
        return contents
...
if hasattr(mw.addonManager, "set_config_help_action"):
    mw.addonManager.set_config_help_action(
        mw.addonManager.addonFromModule(__name__), replace_module_name_in_config_help
    )
